// -----------------------------------------------------------------------------------------------------
// 'MINIMAL 64 Home Computer' emulator written by Carsten Herting (slu4) 2023, last update Apr 10th 2023
// -----------------------------------------------------------------------------------------------------
...
// - Insert the the following into the 'keyPressed()' function: println(key, keyCode, keyCode == CODED);
// - Press and note key codes as shown in the console window.
// - Identify corresponding PS2 scan codes in the provided map 'PS2_Scancodes_Set_2.png'.
// - Update 'ps2ScanCodes' entries in the 'setup()' function below.
...
int DoInstruction() // handle all instructions
{
  ...
  switch(inst) // process the instruction
  {
    ...
    case 4: // INP
      if (serialInput.size() > 0 && serialWaitClocks <= 0) { mA = ps2Input.remove(0); serialWaitClocks = 264; }
      else mA = 0xff;
      break;
    case 5: // INK
