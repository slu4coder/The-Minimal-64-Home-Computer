...
; MIN's MEMORY LAYOUT:
; 0x0000 - 0x0fff   4KB text editor
; 0x1000 - 0x7fff   28KB MIN source file / tokenized file is approx. half the size
; ---------------
; 0x8000 - 0x9dff   8KB MIN interpreter incl. MIN global state (64 bytes)
; 0x9e00 - 0xa0ff   call dict (255 x 3 bytes)
; 0xa100 - 0xa9ff   var dict (255 x 9 bytes)
...
#org 0x8000			LDI 0xfe STA 0xffff                     			; init stack
								JPS _ReadSpace                                ; command line: skip spaces after 'min <filename>'
								...
								CLB g_sub CLB g_halt CLW g_cnt
...
LoadFileTo:  		     	JPS _FindFile CPI 1 BEQ lf_found   	          ; PtrA0..2 now points to file in active FLASH
												BFF LDI 0 RTS															  ; return a failure (file not found)
	lf_found:           LDI 22 ADW _PtrA JPS OS_FlashA                ; search for bytesize of file
                      LDR _PtrA STA _PtrB+0 INW _PtrA JPS OS_FlashA ; bytesize -> PtrB (PtrA now points to data)
                      LDR _PtrA STA _PtrB+1 INW _PtrA JPS OS_FlashA
  lf_loadloop:        DEW _PtrB BCC lf_success                      ; done with copying?
                        LDR _PtrA BFF															  ; load byte from FLASH, switch FLASH off
                        STR _PtrD                                   ; store byte in RAM
                        LDA _PtrA+2 BNK                             ; switch FLASH on
                        INW _PtrD INW _PtrA JPS OS_FlashA
												JPA lf_loadloop
...
OS_FlashA:      LDA _PtrA+1 RL5 ANI 0x0f                            ; is something in the upper nibble?
                CPI 0 BEQ fa_rts
                  ADB _PtrA+2 BNK                                   ; there was something -> update bank register PtrA+2
                  LDI 0x0f ANB _PtrA+1                              ; clear upper nibble of PtrA+1
  fa_rts:     	RTS
...
SourceError:		JPS PrintEnter
  ...
  se_start:     LDR intA PHS JPS _PrintChar PLS
	...
	PrintEnter:		LDI 10 PHS JPS _PrintChar PLS RTS
...
LoadFile:				LDA _PtrD+0 STR g_nextsrc INW g_nextsrc				; emplace first source addr into "sources" vector
	...
	returnchar:			LDI 10 PHS JPS _PrintChar PLS
									JPA _Prompt
		...
		loader_use:			INW g_pc LDR g_pc CPI 's' BNE loader_not
											INW g_pc LDR g_pc CPI 'e' BNE loader_not
												INW g_pc JPS Next LDR g_pc CPI '"' BNE loader_not
													INW g_pc LDA g_nextsrc+1 CPI 0xd0 BCS loader_full
														...
														JPA loader_while
	loader_full:	LDI <MaxFileMsg PHS LDA >MaxFileMsg PHS
								JPS _Print PLS PLS
								JPA _Prompt
	loader_rts:		RTS
...
int_tostr:			CLB int_str
	...
	int_done:		  DEB intcount BNE int_shift
                  LDA intC+2 ANI 0x7f								; erase a possible stored carry
...
fac_next6:			CPI '(' BNE fac_error													; (..) EXPRESSION
...
Term:						JPS Factor
	term_while:		LDR g_pc CPI '*' BEQ term_mult
									CPI '/' BNE term_rts
										...
										LDA intB+0 ORA intB+1 CPI 0 BNE term_okay
...
Expr:						LDR g_pc CPI 0xd9 BEQ expr_not
	...
	expr_not:			INW g_pc																			; consume 'not'
		...
		expr_reuse:	LDA intA+0 STR g_sp LDA intA+1 STR g_spi			; store result as int
								CLB g_refset LDI 2 STA g_type
								CLB g_cnt+1 LDI 1 STA g_cnt+0
	expr_while:		LDR g_pc CPI 0xda BCC expr_rts
									BNE expr_or
										INW g_pc
                  	JPS get LDA intA+0 PHS LDA intA+1 PHS			; push A
                  	JPS RelExpr JPS get
                  	PLS ANB intA+1 PLS ANB intA+0							; AND
										JPA expr_reuse
	expr_or:			CPI 0xdb BNE expr_xor
									INW g_pc
                  JPS get LDA intA+0 PHS LDA intA+1 PHS				; push A
                  JPS RelExpr JPS get
                  PLS ORB intA+1 PLS ORB intA+0								; OR
									JPA expr_reuse
	expr_xor:			CPI 0xdc BNE expr_shiftl
									INW g_pc
                  JPS get LDA intA+0 PHS LDA intA+1 PHS				; push A
                  JPS RelExpr JPS get
                  PLS XRA intA+1 STA intA+1										; XOR
									PLS XRA intA+0 STA intA+0
									JPA expr_reuse
...
TypedCompExpr:	JPS CompExpr																	; parses compound expression of any type
	...
	tcompcntchk:		LDA g_cnt+0 ANI 0xfe ORA g_cnt+1						; error for g_cnt > 1
...
CallStmt:				JPS AssertORound
	...
	callint:			INW g_pc
								LDR g_pc STA callto+0 INW g_pc
								LDR g_pc STA callto+1 INW g_pc
								LDR g_pc CPI 0xd0 BNE callerror								; backwards offset is TN_INT_CONST
									INW g_pc
									LDR g_pc SBB 0xffff
									JPS																					; jump to subroutine
	callto:					0xffff STA 0xff02														; store accumulator in 0xff02
									LDR g_pc ADB 0xffff													; restore the stack pointer
									LDI 2 ADW g_pc															; consume rest of "offset"
									JPS AssertCRound
									RTS

ReturnStmt:			LDR g_pc CPI 0xe0 BCS return_else
									JPS CompExpr LDI 4 ORB g_halt			; it is important to set halt *after* CompExpr()!
									LDA g_sp+0 STA g_retsp+0
									LDA g_sp+1 STA g_retsp+1
									RTS
	return_else:	LDI 4 ORB g_halt CLW g_cnt					; do not return anything after plain "return" statement
								RTS

WhileStmt:			LDA g_pc+0 PHS LDA g_pc+1 PHS
	whileloop:		JPS Expr JPS get
								LDA intA+0 ORA intA+1 CPI 0 BEQ whilebreak
									JPS Block
									LDA g_halt CPI 0 BNE whilebreak
										LDS 1 STA g_pc+1 LDS 2 STA g_pc+0
										JPA whileloop
	whilebreak:		PLS PLS
								JPS SkipStmt
								LDI 0xfd ANB g_halt								; g_halt &= ~BREAK;
								RTS

IfStmt:					JPS Expr JPS get LDA intA+0 ORA intA+1 PHS
								CPI 0 BEQ if_1_else
	...
	if_while:			LDR g_pc CPI 'F' BNE if_1_end
		...
		if_2_else:			JPS Expr JPS get LDA intA+0 ORA intA+1 STS 1
										CPI 0 BEQ if_3_else
											JPS Block JPA if_while
			if_3_else:		JPS SkipStmt JPA if_while
...
FunctionCall: 			LDA g_nextvar+0 PHS LDA g_nextvar+1 PHS
	...
	fun_while:				LDR g_pc CPI ')' BEQ fun_whileex			; parse for callee's parameter )
		...
		fun_typeok:				SBI '0' TAX INW g_pc								; park type in X, consume type
											LDR g_pc CPI '&' BNE fun_plainvar		; reference & variable
												...
												LDR g_pc CPI 'V' BNE fun_typeerr
													...
													INW g_nextvar TXA STR g_nextvar							; store X ->type
		...
		fun_plainvar:			CPI 'V' BNE fun_typeerr
												...
												LDA g_pc+0 STS 11 LDA g_pc+1 STS 10					; g_pc -> par
												LDS 2 STA g_pc+0 LDS 1 STA g_pc+1						; arg -> g_pc
												TXA PHS JPS TypedCompExpr										; request with type again
	...
	fun_whileex:			INW g_pc															; consume callee's param )
										...
										LDA g_halt CPI 0 BEQ fun_nohalt
											LDI 0xfb ANB g_halt									; clear RETURN flag
											LDA g_cnt+0 ORA g_cnt+1 CPI 0 BEQ fun_rts
												LDA g_sp+0 STA fun_dst+0					; move return data
...
VarAssignment:			LDR g_pc CPI '[' BEQ ass_element
	...
	ass_operator:			LDR g_pc CPI '=' BNE ass_fastaddsub
		...
		ass_fastaddsub:	TAY INW g_pc LDR g_pc CPI 0xd0 BNE ass_error	; TN_INT_CONST -> Y
											INW g_pc
											LDS 6 STA assptr+0 LDS 5 STA assptr+1   		; vp->type
											LDR assptr TAX
											PLS CPI 0xff BEQ ass_nooffa
												STA intA+1 PLS STA intA+0									; intA = offset
												LDI 1 CPX BEQ ass_achar
			...
			ass_offaset:		LDI 1 CPX BEQ ass_aachar
												LDR intA STA intB+0 INW intA
												LDR intA STA intB+1
												TYA CPI 'a' BNE ass_fsubint
				...
				ass_fsubint:		CPI 's' BNE ass_error
													...
													DEW intA LDA intB+0 STR intA RTS
				ass_aachar:		TYA CPI 'a' BNE ass_fsubchar
...
VarDefinition:			LDR g_pc CPI 'V' BEQ var_okay
	...
	var_okay:					INW g_pc LDR g_pc STR g_nextvar			; consume 'V', read & store id
										...
										LDA g_sub ORA g_mind DEC BCC var_writesub
											LDA g_sub
...
SimpleLine:			LDR g_pc
	...
	linenext1:		CPI 'B' BNE linenext2
									INW g_pc LDI 2 ORB g_halt JPA linecont					; BreakStmt: g_halt |= BREAK
  ...
  linenext7:    CPI 'P' BNE linenext8
	...
	linecont:			LDR g_pc CPI 0xe0 BCS lineindent									; end of the line (indent) reached?
									LDA g_halt CPI 0 BEQ SimpleLine	                ; any halt flag set?
										RTS
...
Statement:			LDA g_mind CPA g_tind BEQ stmtnormal
	...
	stmtnormal:		LDR g_pc CPI 'I' BNE stmtnext1
		...
		stmtsimple:	JPS SimpleLine
								RTS
	stmtblockend:	LDI 1 ORB g_halt																	; set BLOCKEND bit 0
								RTS
...
Block:					LDA g_nextvar+0 PHS LDA g_nextvar+1 PHS							; save variable and stack state
	...
	blockwhile:			JPS Statement LDA g_halt CPI 0 BEQ blockwhile
									DEB g_tind JPA blockend														; indentation -1
	blocksimple:	JPS SimpleLine
	blockend:			LDI 0xfe ANB g_halt																	; clear BLOCKEND flag (bit 0)
...
FastBlock:			LDR g_pc CPI 0xe0 BCC fblocksimple									; is there an indent? => "hanging" block
									NOT DEC STA g_mind INW g_pc												; consume indentation mark
									INB g_tind																				; indentation +1
	fblockwhile:			JPS Statement LDA g_halt CPI 0 BEQ fblockwhile
									DEB g_tind																				; indentation -1
									LDI 0xfe ANB g_halt																; clear BLOCKEND flag (bit 0)
									RTS
...
g_halt:					0xff					; halt flags ... stop processing of statements
...
#org 0x1000     file:        	; beginning of the source file (editor text file)
#org 0x9e00			firstcall:		; call dictionary 3 bytes * 256 = 0x300 bytes
#org 0xa100     firstvar: 	  ; var dictionary  9 bytes * 256 = 0x900 bytes
#org 0xaa00			firstsp:			; data memory stack (1408 bytes)
#org 0xaf80			endsp:				; data memory end
#org 0xaf92			firstsrc:			; vector of imported source files (22bytes * 5 = 110 bytes, max. 5 entries)

#org 0xbf75     _PtrA:        ; lokaler OS pointer (3 bytes) used by LoadFileTo routine
#org 0xbf78     _PtrB:        ; lokaler OS pointer (3 bytes)
#org 0xbf7e     _PtrD:        ; lokaler pointer (2 bytes)
